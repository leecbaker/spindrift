...
async fn table_wrapper_avoids_active_left_float() {
    ...
    assert!(blue.x >= 39.0, "table should avoid active float: {blue:?}");
}
    ...
    assert!(
        document.pages[0]
            ...
            .any(|rect| rect.fill == Some(Color::new(0, 128, 0))
                && (rect.height - 140.0).abs() < 0.01),
        "oversized flex container should be laid out on the first page until flex fragmentation is implemented"
    );
}
...
async fn inline_block_fragment_lays_out_atomic_inline_children_in_one_line() {
    ...
    assert!((blue.x - 260.0).abs() < 0.01);
    assert!(red.x < 160.0, "inline spaces should affect right alignment");
    assert!(
        green.x - red.x - 50.0 > 2.5,
...
async fn absolute_flex_children_use_flex_static_position_and_ignore_justify_self() {
    ...
    for (container, child) in yellow_rects.iter().zip(teal_rects.iter()) {
        ...
        assert!((child.y + child.height - (container.y + container.height - 1.5)).abs() < 0.01);
    }
}
...
#[tokio::test]
async fn shrink_to_fit_inline_block_uses_stable_line_width_epsilon() {
    let document = Html::from_file_async("weasyprint-samples/ticket/ticket.html")
        .await
        .unwrap()
        .render_async(&RenderOptions::default())
...
#[tokio::test]
async fn flex_item_text_line_fit_uses_stable_width_epsilon() {
    let document = Html::from_file_async("weasyprint-samples/ticket/ticket.html")
        .await
        .unwrap()
        .render_async(&RenderOptions::default())
...
async fn inline_block_preserves_explicit_collapsed_spaces() {
    ...
    assert!(after.x - (background.x + background.width) > 2.0);
}
...
async fn inline_block_lays_out_block_children_as_atomic_fragment() {
    ...
    assert!(two.y < one.y);
    assert!(after.x > background.x + background.width);
}

...
fn run_quire(logging_flag: Option<&str>, rust_log: Option<&str>) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_quire"));
    command
    ...
    command.output().expect("quire binary should run")
}

fn stderr(output: &Output) -> String {
    String::from_utf8(output.stderr.clone()).expect("quire should write UTF-8 diagnostics")
}

#[test]
fn default_logging_reports_a_stylesheet_error_once() {
    let output = run_quire(None, None);
    let diagnostic = stderr(&output);

    assert!(!output.status.success());
    assert!(diagnostic.contains(" ERROR quire]"));
    assert_eq!(diagnostic.lines().count(), 1);
}

#[test]
fn verbose_logging_omits_debug_messages() {
    let output = run_quire(Some("--verbose"), None);
...
fn debug_logging_includes_debug_messages() {
    let output = run_quire(Some("--debug"), None);
...
fn rust_log_controls_logging_without_a_cli_flag() {
    let output = run_quire(None, Some("debug"));
...
fn quiet_logging_ignores_rust_log() {
    let output = run_quire(Some("--quiet"), Some("trace"));

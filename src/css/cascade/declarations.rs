    ...
    match name {
        "place-content" => {
            let (align, justify) = split_place_shorthand(
                value,
                parse_align_content_keyword,
                parse_justify_content_keyword,
            )?;
...
fn content_alignment(
    ...
    match safety {
        AlignmentSafety::Unsafe => ContentAlignment::new(keyword),
...
fn self_alignment(keyword: SelfAlignmentKeyword, safety: AlignmentSafety) -> SelfAlignment {
    match safety {
        AlignmentSafety::Unsafe => SelfAlignment::new(keyword),
...
fn parse_content_alignment_keyword(
    ...
    if safety == AlignmentSafety::Safe && !alignment_safety_allowed_for_content(keyword) {
        return None;
    }
...
fn parse_self_alignment_keyword(
    ...
    if safety == AlignmentSafety::Safe && !alignment_safety_allowed_for_self(keyword) {
        return None;
    }
...
fn logical_box_axis_side_names(name: &str) -> Option<[&'static str; 2]> {
    match name {
        "margin-block" | "padding-block" => Some(["block-start", "block-end"]),
        "margin-inline" | "padding-inline" => Some(["inline-start", "inline-end"]),
        _ => None,
    }
}
...
fn property_is_inherited(name: &str) -> bool {
    matches!(
        name,
        "border-collapse"
            | "border-spacing"
...
const ALL_MODELED_LONGHANDS: &[&str] = &[
    "display",
    "flex-direction",
    "justify-content",
    "justify-items",
...
fn copy_modeled_property(style: &mut ComputedStyle, source: &ComputedStyle, name: &str) {
    match name {
        ...
        "background-position" => style.background_position = source.background_position,
        "background-repeat" => style.background_repeat = source.background_repeat,
        "background-origin" => style.background_origin = source.background_origin,
        "background-clip" => style.background_clip = source.background_clip,
        ...
        match name {
            ...
            "background-image" => {
                style.background_image =
                    parse_background_image(value, declaration.base_url, declaration.root_url);
            }
            "background-size" => {
                if let Some(size) = parse_background_size(value, style.font_size) {
                    style.background_size = size;
                }
            }
            "background-position" => {
                if let Some(position) = parse_background_position(value, style.font_size) {
            ...
            "background-origin" => {
                if let Some(box_) = parse_background_box(value) {
            ...
            "vertical-align" => {
                style.vertical_align = match value.to_ascii_lowercase().as_str() {

use super::*;
use crate::layout::assets::{apply_object_fit, raster_image_interpolation, svg_replaced_group};
use std::rc::Rc;
...
impl<'a> LayoutBuilder<'a> {
    ...
    pub(in crate::layout) fn prepare_inline_line_fragment(
        ...
        let line_geometry = InlineLineGeometry::new(
            self.content_left,
            self.content_right,
            self.cursor_y,
    ...
    pub(in crate::layout) fn paint_prepared_inline_atom(&mut self, prepared: &PreparedInlineAtom) {
        ...
        if !fragment.is_empty() {
            ...
            if matches!(atom.content(), InlineAtomContent::Image(_))
                && self.root_principal_flow_context.active_body.is_some()
                && self.principal_flow.has_propagated_axes()

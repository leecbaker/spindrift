...
impl<'a> LayoutBuilder<'a> {
    ...
    pub(in crate::layout) fn layout_block_with_descendant_percentage_height_basis(
        ...
        loop {
            ...
            let current_fragmentainer = Fragmentainer::new(
                self.page_area_height(),
                self.cursor_y + geometry.relative_offset.y() - self.page_bottom(),
            );
            let empty_destination_fragmentainer = match fragmentainer_kind {
                FragmentainerKind::Page => {
                    ...
                    Fragmentainer::new(next_context.area_height(), next_context.area_height())
                }
                FragmentainerKind::Column => {
                    ...
                    Fragmentainer::new(next_capacity, next_capacity)
                }
            };
            let should_prebreak = !fragments_as_promoted_spanner
        ...
        let applied_start_margin =
            page_start_margin(layout_pt(geometry.style.margin.top), starts_at_page_top);
        ...
        let establishes_independent_bfc = geometry
            ...
            || geometry.style.contain.layout
            || geometry.style.contain.paint
                ...
                geometry = self.block_layout_geometry_in_inline_span(
                    ...
                    BlockLayoutInlineConstraint {
                        ...
                        auto_border_box_width: (placement.available_width
                            < containing_inline_size - FLOAT_EPSILON)
                            .then_some(border_box_pt(placement.available_width)),
                    },
                );
                if placement.available_width < containing_inline_size - FLOAT_EPSILON {
        ...
        let definite_content_height = geometry
            .definite_content_height
            .map(PhysicalContentHeight::points);
        ...
        let content_logical_inline_size = geometry.content_logical_inline_size().points();
        let built_multicol_child_boxes;
        let child_boxes = if child_boxes.is_none()
            && (style.column_count.is_some()
                || matches!(style.column_width, css::ComputedColumnWidth::Length(_))
        ...
        if std::env::var_os("QUIRE_TRACE_FLOATS").is_some() && element.tag == "h4" {
            eprintln!(
                "h4 block entry left={} right={} cursor={}",
                previous_left, previous_right, self.cursor_y,
            );
        }
        self.content_left = inner_x;
        self.content_right = inner_x + inner_width;
        let is_document_canvas = is_document_canvas_element(element);
        ...
        self.child_available_space_stack
            .push(child_available_space_for_block(
                ...
                self.current_page_context
                    .logical_block_size(style.writing_mode),
            ));
        if establishes_independent_bfc {
        ...
        let positioned_containing_block_scope =
            if let Some(mode) = positioning_containing_block_mode {
                ...
                let containing_block = ContainingBlock::from_page_top_rect(
                    geometry.padding_box_top_rect(outer_x, block_top, positioning_content_height),
                );
        ...
        let paint_containment_applies =
            style.contain.paint && property_containment_applies_to_element(element, style);
        // Scroll containers must retain their un-clipped child geometry until
        // the final paint effect: unsafe alignment can intentionally place
        // content on the start side of the scrollport. A non-scrollable
        // `hidden`/`clip` overflow box keeps the eager primitive clip used by
        // the legacy paint stream.
        let overflow_requires_deferred_geometry = matches!(
            effective_overflow_for_style(style),
            css::Overflow::Auto | css::Overflow::Scroll
        ) || style.scroll_snap_type
            != css::ScrollSnapType::None;
        ...
        let overflow_clip_active = if used_overflow_clips
            ...
            && !paint_containment_applies
            && !(needs_deferred_overflow_clip && overflow_requires_deferred_geometry)
        {
        ...
        let auto_overflow_has_block_descendants = child_boxes.is_some_and(|children| {
        ...
        let suppresses_descendant_fragmentation = (used_overflow_clips
            && (definite_content_height.is_some() || auto_overflow_has_block_descendants))
        ...
        if suppresses_descendant_fragmentation {
            // Scroll containers with a definite size or block descendants,
            // an unbreakable line in a zero-height column, and size-contained
            // column subjects establish monolithic outer-flow boxes. Visible
            // overflow from size-contained descendants is retained with that
            // principal fragment without contributing to its used size.
            // <https://www.w3.org/TR/css-contain-1/#containment-size>
            // <https://www.w3.org/TR/css-break-3/#monolithic>
            self.fragmentation_suppression_depth += 1;
        }
        if style.contain.layout {
            // Forced breaks remain usable by a nested fragmentation context,
            // but must not escape through this layout-containment boundary to
            // the fragmentainer that contains the box.
            // <https://www.w3.org/TR/css-contain-1/#containment-layout>
            self.forced_break_containment_scopes
                .push(self.fragmentainer_override);
        }
        ...
        let has_collectable_inline_content =
            inline_text_has_non_phantom_content(&text, style) || has_generated_inline_content;
        ...
        if style.contain.layout {
            self.forced_break_containment_scopes.pop();
        }
        ...
        if std::env::var_os("QUIRE_TRACE_FLOATS").is_some() && element.tag == "h4" {
            eprintln!(
                "h4 block exit left={} right={} cursor={}",
        ...
        if definite_content_height.is_some()
        ...
        let deferred_rounded_overflow_clip = deferred_overflow_clip.and_then(|_| {
            ...
            rounded_clip_rect_for_box(border_paint_rect, style, border_widths, reference_box)
        });
        if block_height > 0.0 {
        ...
        if propagates_document_canvas_properties {
        ...
        } else if fragmented_spanner_slices.is_empty()
            ...
            && style.visibility == Visibility::Visible
        {
            own_background_primitives = self.box_background_primitives(border_paint_rect, style);
        }
        if fragmented_spanner_slices.is_empty()
            && fragmented_block_slices.is_empty()
            && border_paint_rect.size.width > 0.0
            && border_paint_rect.size.height > 0.0
            && style.visibility == Visibility::Visible
            ...
            for layer in &mut self.positioned_layers[positioned_layer_start..] {
                layer.context.effects.overflow_clip = Some(
                    layer
                        ...
                        .effects
                        .overflow_clip
                        .and_then(|existing| existing.intersect(overflow_clip))
                        .unwrap_or(overflow_clip),
                );
            }
        }
            ...
            && !vertical_block_align_content_needs_fragment_bounds(style)
        {
            ...
            if let Some(overflow_clip) = deferred_overflow_clip {
                fragment = if let Some(rounded_clip) = deferred_rounded_overflow_clip {
                    fragment
                    ...
                    fragment
                        .with_primitives_clipped_to_rect_preserving_structure(overflow_clip)
                        .with_contents_effect_scoped_to_rect(overflow_clip)
                };
            }
        ...
        let suppress_leading_empty_block_fragment = fragmentainer_kind == FragmentainerKind::Page
            && has_auto_height(style)
            && !has_direct_inline_content
        ...
        if self.active_fragmentainer_kind() == FragmentainerKind::Column {
            ...
            for page_index in 0..plan.pages_to_push {
                ...
                if is_last_conceptual_fragment {
                    self.cursor_y -= plan.last_fragment_used_block_size;
                } else {
            ...
            if plan.has_unmaterialized_tail {
                ...
                self.cursor_y = self.page_top() - plan.last_fragment_used_block_size;
            }
            return;
        }
        let available = (self.cursor_y - self.page_bottom()).max(0.0);
    ...
    fn layout_definite_block_with_deferred_descendant_overflow(
        ...
        let border_box_height = definite_content_height + vertical_extras;
        let block_top = self.cursor_y;
    ...
    boxes.is_some_and(|boxes| {
        boxes.iter().any(|box_| match box_ {
            ...
            box_tree::FormattingBox::InlineSplitBlockContext(context) => {
                formatting_boxes_have_forced_break_in(Some(&context.children), fragmentainer_kind)
            }

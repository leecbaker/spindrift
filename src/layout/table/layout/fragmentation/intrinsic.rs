//! Intrinsic table sizing, inline-table construction, and height estimates.
...
impl<'a> LayoutBuilder<'a> {
    ...
    pub(in crate::layout) fn table_wrapper_flex_sizing_from_fragment(
        ...
        self.with_speculative_layout(|layout| {
            let (grid_min_content, grid_max_content) = layout.table_intrinsic_widths_from_fragment(
                element,
                style,
                stylesheets,
                fragment,
            ...
            let (_, wrapper_preferred) = layout
            ...
            let wrapper_intrinsic_block = (layout.estimate_table_height(
                element,
    ...
    pub(crate) fn inline_table_atom_for_element(
        ...
        // <https://drafts.csswg.org/css-tables-3/#table-layout>
        // <https://drafts.csswg.org/css-display-3/#valdef-display-inline-table>
        let measurement = crate::layout::builder::SpeculativeLayoutTransaction::begin(self);
        let grid = table_grid(rows);
        let available_width =
        ...
        atom_style.border_width_values =
    ...
    pub(crate) fn estimate_table_height(
        ...
        let height = self.with_speculative_layout(|layout| {
            let input = TableLayoutInput::from_fragment(fragment);
            let rows = input.row_ordering.rows.as_slice();
            ...
            let available_table_width =
                (available_outer_width - style.margin.left - style.margin.right)
                    .max(style.font_size);
            let mut table_width = used_table_width(style, available_table_width, None);
            if rows.is_empty() {
                return layout.estimate_empty_table_height(
                    captions,
                    style,
                    stylesheets,
                    available_table_width,
            ...
            let collapsed_geometry =
                (table_metrics.border_collapse == css::BorderCollapse::Collapse).then(|| {
                    layout.collapsed_table_geometry(
                        rows,
                        &grid,
                        style,
                        stylesheets,
            ...
            // grid's content span.
            // <https://drafts.csswg.org/css-tables-3/#table-layout>
            layout.resolve_table_used_content_inline_size(
                rows,
                &grid,
                style,
                stylesheets,
            ...
            let column_plan = layout.table_column_plan(
                rows,
                &grid,
                style,
                stylesheets,
            ...
            total += layout.estimate_table_captions_height(
                captions,
                style,
                stylesheets,
            ...
            let table_used_style = layout.table_used_style(style);
            let table_context = TableGridLayoutContext {
                rows,
                grid: &grid,
                table_style: &table_used_style,
            ...
            let table_height_plan = layout.table_height_plan(&table_context);

...
use crate::css::{self, CssColor, FontFamily, FontVariationSettings};
...
use crate::document::paint::paths::{
    ...
    RenderedPathLineJoin, RenderedPathPaint, RenderedPathPaintOrder, RenderedSvgPathPattern,
};
...
pub(crate) struct SvgPresentationOverride {
    ...
    /// Resolved host-CSS spacing in SVG/CSS pixels.
...
/// Host CSS presentation overrides for a standalone inline SVG resource.
#[derive(Debug, Clone, Default)]
...
pub(crate) struct SvgAsset {
    ...
    /// Accessible source strings for rendered SVG `<text>` elements, in tree
    /// order. `usvg`'s layout chunks may omit a collapsed boundary space at a
    /// `tspan` edge, while PDF `/ActualText` must retain the source text that
    /// users select and copy.
    text_actual_texts: Vec<Rc<str>>,
    source: Rc<[u8]>,
}
...
impl SvgAsset {
    ...
    pub(crate) fn with_view_fragment(&self, fragment: Option<&str>) -> Self {
            ...
            && let Some(view) = self.view_fragments.get(fragment)
        {
            ...
            {
                asset.tree = tree;
                asset.text_actual_texts = svg_actual_texts(&source);
            }
        }
        asset
    }
    ...
    pub(crate) fn paint_paths_for_source_rect(
        ...
        let mut actual_texts = self.text_actual_texts.iter().cloned().collect();
...
/// SVG text lowered by `usvg` to a complete visual scene.
///
/// The PDF writer wraps that scene in one `/ActualText` marked-content span,
/// preserving images, clipping, and compositing without adding native text.
#[derive(Debug, Clone, PartialEq)]
pub(crate) struct SvgOutlinedText {
    pub(crate) content: SvgPaintGroup,
...
pub(crate) enum SvgPaintItem {
    ...
    /// SVG text lowered by upstream `usvg` and paired with PDF ActualText.
        ...
        for item in &mut self.items {
            match item {
                ...
                SvgPaintItem::OutlinedText(outlined) => {
                    let nested = std::mem::replace(&mut outlined.content, SvgPaintGroup::empty());
                    outlined.content = nested.with_clip(clip.clone());
                }
            }
        }
        self
    ...
    fn into_paths(self) -> Vec<RenderedPath> {
        ...
        for item in self.items {
            match item {
                ...
                SvgPaintItem::OutlinedText(outlined) => paths.extend(outlined.content.into_paths()),
            }
        }
        paths
    }
    ...
    fn transformed(mut self, transform: PaintTransform) -> Self {
        for item in &mut self.items {
            match item {
                ...
                SvgPaintItem::OutlinedText(outlined) => {
                    let nested = std::mem::replace(&mut outlined.content, SvgPaintGroup::empty());
                    outlined.content = nested.transformed(transform);
                }
            }
        }
        self
    ...
    pub(crate) fn raster_images<'a>(&'a self, images: &mut Vec<&'a RenderedImage>) {
        for item in &self.items {
            match item {
                ...
                SvgPaintItem::Path(_) => {}
                SvgPaintItem::OutlinedText(outlined) => outlined.content.raster_images(images),
            }
        }
    }
}
...
fn canonicalize_svg_paint_group(
    ...
    for item in &mut group.items {
        match item {
            ...
            SvgPaintItem::RasterImage(_) => {}
            SvgPaintItem::OutlinedText(outlined) => {
                canonicalize_svg_paint_group(&mut outlined.content, servers)
            }
        }
    }
}
...
fn collect_svg_group(
    ...
    filter_taint: &SvgFilterTaintCatalog,
    actual_texts: &mut VecDeque<Rc<str>>,
) -> SvgPaintGroup {
...
/// Collect one normalized SVG group and preserve its compositing boundaries.
    ...
    for node in group.children() {
        match node {
            ...
            usvg::Node::Text(text) => {
                ...
                let actual_text = actual_texts.pop_front().unwrap_or_else(|| {
                    text.chunks()
                        .iter()
                        .map(usvg::TextChunk::text)
                        .collect::<String>()
                        .into()
                });
                if !flattened.items.is_empty() && !actual_text.is_empty() {
                    rendered
                        .items
                        .push(SvgPaintItem::OutlinedText(Box::new(SvgOutlinedText {
                            content: flattened,
                            actual_text,
                        })));
                }
...
/// Upper bound for one SVG text/filter fallback surface.
const MAX_SVG_EFFECT_PIXELS: u64 = 16 * 1024 * 1024;
/// Upper bound for SVG convolution work, including every output sample.
...
fn svg_effect_paths_and_text(group: SvgPaintGroup) -> Option<(Vec<RenderedPath>, String)> {
    ...
    for item in group.items {
        match item {
            SvgPaintItem::Path(path) => paths.push(*path),
            SvgPaintItem::OutlinedText(outlined) => {
                let (child_paths, _child_text) = svg_effect_paths_and_text(outlined.content)?;
            ...
            SvgPaintItem::RasterImage(_) => return None,
        }
    }
...
fn render_svg_image(
    ...
    match image.kind() {
        ...
        usvg::ImageKind::SVG(tree) => {
            ...
            let mut nested_actual_texts = VecDeque::new();
...
fn svg_pattern(pattern: &usvg::Pattern, opacity: f32) -> Option<RenderedSvgPathPattern> {
    ...
    let mut pattern_actual_texts = VecDeque::new();
...
fn parse_svg_bytes_with_optional_image_context_and_filter_taint(
    ...
    let text_actual_texts = svg_actual_texts(&normalized_source);
    Ok(SvgAsset {
        tree,
        filter_taint,
        viewport_background,
...
fn parse_svg_tree(bytes: &[u8], default_size: usvg::Size) -> Result<usvg::Tree, String> {
    ...
    let options = {
        ...
        options.fontdb = svg_system_fontdb();
        options
    };
    usvg::Tree::from_data(bytes, &options).map_err(|error| error.to_string())
}

/// Collect the copy-and-selection text of every source SVG `<text>` element
/// in document order. SVG's default white-space processing collapses runs of
/// XML whitespace, but a collapsed boundary between a text node and a
/// `<tspan>` is still a source character. Upstream `usvg` omits that boundary
/// from its layout chunk text, so deriving `/ActualText` from the normalized
/// source preserves the accessible string independently of visual outlining.
fn svg_actual_texts(source: &[u8]) -> Vec<Rc<str>> {
...
/// The visual SVG-text fallback deliberately uses one immutable native font
/// database. It is initialized once because SVG assets may be reparsed for
/// CSS object sizing, fragments, and viewports.
#[cfg(not(target_arch = "wasm32"))]
fn svg_system_fontdb() -> Arc<usvg::fontdb::Database> {
    ...
    Arc::clone(FONTDB.get_or_init(|| {
        ...
        database
            ...
            .expect("the checked-in Ahem test font must load");
        Arc::new(database)
    }))
}
...
/// Serialize a computed CSS `font-family` list for the standalone SVG parser.
...
/// normalization payload. Values remain CSS numbers for `usvg` to resolve.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 100"><rect width="200" height="100"/></svg>"#,
    ...
    fn solid_svg_masks_rasterize_the_retained_subtree_instead_of_painting_it_unmasked() {
        ...
        let scene = asset.paint_inline_group(paint_rect(0.0, 0.0, 15.0, 7.5), true);

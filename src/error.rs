/// Spindrift's standard result type.
///
/// ```no_run
/// use spindrift::{Html, PdfOptions, RenderOptions, Result};
/// use std::fs::File;
///
...
///
/// ```no_run
/// use spindrift::{Error, Html, PdfOptions, RenderOptions};
/// use std::fs::File;
///
...
pub enum Error {
    ...
    #[error("{0}")]
    /// Input that Spindrift cannot process.
    InvalidInput(String),

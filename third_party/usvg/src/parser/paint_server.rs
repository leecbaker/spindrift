...
fn node_to_user_coordinates(
    ...
    match node {
        ...
        Node::Text(text) => {
            // QUIRE: parser-only text retention leaves layout-derived text
            // bounds empty. Object-bounding-box paint servers must therefore
            // remain in their normalized source coordinate system until
            // Quire has shaped the text and can supply the real glyph bounds.
            // Processing them against the empty placeholder bbox would
            // discard the fill/stroke entirely.
            #[cfg(feature = "text")]
            if text.layouted.is_empty() {
                return;
            }

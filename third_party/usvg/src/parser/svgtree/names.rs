...
impl AId {
    pub(crate) fn from_str(text: &str) -> Option<AId> {
        // This source-pinned parser extension deliberately avoids changing
        // the generated perfect-hash table. `text-shadow` is retained as a
        // raw normalized value and parsed by Quire's document CSS pipeline.
        if text == "text-shadow" {
            Some(AId::TextShadow)
        } else if text == "data-quire-text-typography-key" {
            Some(AId::QuireTextTypographyKey)
        } else {
    ...
    #[inline(never)]
    pub fn to_str(self) -> &'static str {
        if self == AId::TextShadow {
            "text-shadow"
        } else if self == AId::QuireTextTypographyKey {
            "data-quire-text-typography-key"
        } else {
            ATTRIBUTES.key(&self)
        }
    }

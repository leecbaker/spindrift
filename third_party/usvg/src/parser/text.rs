...
pub(crate) fn convert(
    ...
    let direction = match text_node.find_attribute::<&str>(AId::Direction) {
        Some("rtl") => crate::TextDirection::RightToLeft,
        _ => crate::TextDirection::LeftToRight,
    };
    let unicode_bidi = match text_node.find_attribute::<&str>(AId::UnicodeBidi) {
        Some("embed") => crate::TextUnicodeBidi::Embed,
        Some("isolate") => crate::TextUnicodeBidi::Isolate,
        Some("bidi-override") => crate::TextUnicodeBidi::BidiOverride,
        Some("isolate-override") => crate::TextUnicodeBidi::IsolateOverride,
        Some("plaintext") => crate::TextUnicodeBidi::Plaintext,
        _ => crate::TextUnicodeBidi::Normal,
    };
    ...
    // QUIRE: retain normalized text before usvg's optional font lookup and
    // layout stage. The host owns document-scoped font selection, shaping,
    // embedding, and PDF extraction; calling `text::convert` here would
    // create a second font-rendering path and would discard text whenever
    // usvg cannot resolve a system font.
    //
    // Keep the layout-derived fields at their documented empty defaults.
    // Quire consumes only the normalized chunks, styles, positioning lists,
    // and transform above, then shapes them through its FontSystem.
    // See `third_party/usvg/QUIRE_PATCHES.md`.
...
fn collect_text_chunks_impl(
    ...
    for child in parent.children() {
        ...
        let span = TextSpan {
            ...
            text_shadow: parent
                .find_attribute::<&str>(AId::TextShadow)
            ...
            text_typography_key: parent
                .find_attribute::<&str>(AId::QuireTextTypographyKey)
...
fn convert_writing_mode(text_node: SvgNode) -> WritingMode {
        ...
        .find(|n| n.has_attribute(AId::WritingMode))
    {
        match n.attribute(AId::WritingMode).unwrap_or("lr-tb") {
            "tb" | "tb-rl" | "vertical-rl" => WritingMode::VerticalRl,
...
/// Normalizes the inherited SVG/CSS `text-orientation` property without
/// performing any font lookup or glyph layout.
///
...
fn convert_text_orientation(text_node: SvgNode) -> TextOrientation {
    text_node
        .ancestors()
        .find(|node| node.has_attribute(AId::TextOrientation))
        .map_or(TextOrientation::Mixed, |node| {
            match node.attribute(AId::TextOrientation).unwrap_or("mixed") {
                "upright" => TextOrientation::Upright,
                // `sideways-right` is a legacy SVG value with the same
                // interoperable result as CSS `sideways`.

...
#![forbid(unsafe_code)]
// QUIRE: SVG text normalization intentionally bypasses upstream's text layout
// and flattening modules. They remain compiled to preserve the upstream text
// tree API, but are unreachable in this parser-only fork.
#![allow(dead_code)]
#![warn(missing_docs)]

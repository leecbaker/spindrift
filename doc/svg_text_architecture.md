...
Spindrift uses upstream `usvg` to normalize and lay out SVG `<text>` elements.
On native targets, `usvg` receives one lazily initialized system-font database
and lowers SVG text into its ordinary vector paint scene. Spindrift paints that
...
native-text duplicate. HTML text continues to use Spindrift font selection,
...
```text
HTML/CSS text  --> Spindrift FontSystem --> embedded native PDF text
...
order continue through Spindrift's ordinary SVG scene adapter.
...
Spindrift exposes an explicit SVG font-source API.

...
| Formatting-context block contribution | Intrinsic logical block-size requests are dispatched at the shared formatting-context boundary. Nested Flex, Grid, Table, replaced, and ordinary-flow boxes retain their own line, track, wrapper, ratio, and block-stack semantics; the parent adds outer box-model edges once. | Focused nested row/column Flex, Grid track, table-wrapper, horizontal/vertical writing-mode, and compact `request_sex` geometry regressions |
